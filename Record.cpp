...
std::string Record::toString() {
    return std::string().append("学号：").append(this->sno)
    .append(" 通关等级：").append(std::to_string(this->level))
    .append(" 记录序号：").append(std::to_string(this->index))
    .append(" 通关用时：").append(std::to_string(this->time_cost_s))
    .append(" 得分：").append(std::to_string(this->score))
    .append(" 记录时间：").append(this->record_time)
    ;
}
